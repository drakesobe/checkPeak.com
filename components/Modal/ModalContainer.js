      ...
      <div>
        ...
        <div className="text-sm text-gray-300 mt-1">
          ...
          <span>
            Price:{" "}
            <strong className="text-white">
              {priceNumber ? priceNumber.toFixed(2) : "N/A"}
            </strong>
          </span>
        </div>
      </div>
...
export default function ModalContainerSmartstack({
  stack,
  allStacks = [],
  onClose,
}) {
  ...
  const imageUrl =
    ...
    stack?.image ||
    (stack?.rawFields &&
      (stack.rawFields["Nutrition Label URL"] || stack.rawFields.Image)) ||
    "";
  ...
  const servingsNumber =
    parseNumber(safeGet(stack, "Servings")) ||
    parseNumber(stack?.servings) ||
    0;
  const priceNumber =
    parseNumber(safeGet(stack, "Price")) || parseNumber(stack?.price) || 0;
  ...
  const preprocessImage = async () => {
    ...
    try {
      ...
      try {
        ...
        const orientation = await Tesseract.recognize(
          croppedCanvas,
          "eng",
          {
  ...
  const fetchRecords = async (text) => {
    ...
    try {
      ...
      // normalize banned
      const normalizedBanned = (Array.isArray(rawBanned)
        ? rawBanned
        : []
      )
        .map((r) => {
          ...
          if (r.fields) {
            ...
            return {
              ...
              name: (
                ...
                .trim(),
              synonyms:
                f["Synonyms (Extended)"] || f["Synonyms"] || "",
              notes:
                f["Pharmacology Notes"] ||
                f["Notes"] ||
                f["Benefits"] ||
                "",
              benefits: f["Benefits"] || "",
      ...
      // debug
      console.log(
        "check-smartstack: matchedBanned:",
        normalizedBanned.map((x) => x.name)
      );
      console.log(
        "check-smartstack: matchedIngredients:",
        normalizedIngredients.map((x) => x.name)
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (
      imageUrl &&
      cachedText &&
      cachedText !== "No OCR text detected."
    ) {
      setOcrText(cachedText);
      if (RECORDS_CACHE[imageUrl]) {
    ...
    if (imageUrl && img && (img.complete || img.naturalWidth)) {
      setTimeout(() => {
        runOCR(true).catch((e) =>
          console.warn("Auto OCR (effect) failed:", e)
        );
      }, 150);
    }
  ...
  const handleImageLoad = () => {
    ...
    setTimeout(() => {
      ...
      if (!cachedText || cachedText === "No OCR text detected.") {
        runOCR(true).catch((e) =>
          console.warn("Auto OCR (onLoad) failed:", e)
        );
  ...
  const counts = {
    Prohibited: matchedBanned.filter(
      (r) => r.banType === "Prohibited"
    ).length,
    Limited: matchedBanned.filter(
  ...
  const filteredBanned = useMemo(() => {
    ...
    const filtered = arr.filter((rec) => {
      ...
      if (filter === "Prohibited")
        return rec.banType === "Prohibited";
      if (filter === "Limited")
        return rec.banType === "Limited to Out of Competition";
      if (filter === "Other")
        return ![
          "Prohibited",
          "Limited to Out of Competition",
        ].includes(rec.banType);
      return true;
    });
    filtered.sort(
      (a, b) =>
        (severityMap[a.banType] ?? 2) -
        (severityMap[b.banType] ?? 2)
    );
    return filtered;
                      ...
                      <div className="space-y-3">
                        {loadingOCR || loadingRecords ? (
                          ...
                          <p className="text-gray-400 text-sm italic text-center">
                            No banned or monitored substances
                            detected for this filter.
                          </p>
                        )}
                      </div>
            ...
            <div className="mt-4 sticky bottom-0 bg-gray-800 pt-4">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-400">
                  Results powered by your Airtable
                </div>
