 ...
 *
 * Keep in mind:
 *  - This file intentionally keeps the same normalizations, caching and behavior from today's edits.
 *  - It restores Servings and Price values into the header.
 */
...
export default function NutritionModal({ stack, allStacks = [], onClose }) {
  ...
  const runOCR = async (force = false) => {
    ...
    // if OCR cached and forcing, still fetch records
    if (ocrCache[imageUrl] && force) {
      setOcrText(ocrCache[imageUrl]);
      await fetchRecords(ocrCache[imageUrl]);
      return;
