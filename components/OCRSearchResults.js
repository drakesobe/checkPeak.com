 ...
 * Mobile UX:
 *  - Native horizontal scroll (no fades/indicators)
 *  - Momentum scrolling on iOS
 *  - Compact spacing & wrapping controls
 *  - Searching indicator above title (fade in/out)
 *
 * Accessibility:
 *  - Buttons have aria-expanded/pressed
...
export default function OCRSearchResults({
  ...
  // Ingredient highlight color (brand-friendly purple)
    ...
    <div className="w-full max-w-[2500px] mx-auto px-4 sm:px-4 py-6 sm:py-6 font-sans space-y-6 relative">
      <section>
        ...
        <AnimatePresence initial={false}>
          {showSearchingIndicator && (
            <motion.div
              ...
              className="mb-2 flex items-center justify-center sm:justify-start text-gray-700"
              aria-live="polite"
            >
        ...
        <p className="text-sm text-gray-600 mb-4 text-center sm:text-left">
          {matchedSubstances?.length ?? 0} total results — {bannedRecords.length}{" "}
          ...
          <AnimatePresence initial={false}>
            {bannedOpen && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -6 }}
                className="mt-3 relative overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0 scroll-region"
      ...
      >
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3 rounded-t-xl border-t border-gray-200 bg-white/95 backdrop-blur-sm shadow-lg">
            <div
              className="flex items-center gap-2 sm:gap-3 overflow-x-auto py-1 w-full sm:w-auto scroll-region"
              style={{ WebkitOverflowScrolling: "touch" }}
            >
              <button
      ...
      {/* Local styles for highlights, toggles, and clean mobile scrollbars */}
      <style jsx>{`
        .highlight-match {
        ...
        /* Flat, noticeable section toggles */
        .search-toggle-btn {
          display: inline-flex;
          align-items: center;
          gap: 10px;
        ...
        .search-toggle-btn .caret {
          color: #4b5563; /* slightly darker neutral gray */
          font-weight: 600;
        }
        ...
        /* Clean horizontal scrollbar for mobile/desktop.
           Hidden by default on iOS; shows on scroll (native behavior).
           For WebKit we keep it thin and neutral. */
        ...
        .scroll-region::-webkit-scrollbar-thumb {
          background: rgba(107, 114, 128, 0.45); /* neutral gray, slightly darker */
          border-radius: 999px;
        }
        .scroll-region::-webkit-scrollbar-thumb:hover {
          background: rgba(75, 85, 99, 0.65); /* a bit darker on hover */
        }
