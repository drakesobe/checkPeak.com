...
// Full-screen dark day-planner sheet.
// Slides up over today.jsx when the athlete taps "Plan day".
// Self-contained: manages its own date, planner events, and workout/nutrition
// data loading. Receives classSchedules as a prop (shared with today.jsx).
"use client";
...
function dayPattern(days) {
  ...
  return [...days].sort((a, b) => (a === 0 ? 7 : a) - (b === 0 ? 7 : b)).map(d => S[d] || "?").join("/");
}
...
function buildNutritionDefaults(mealBlocks) {
  return Object.entries(MEAL_DEFAULTS).map(([key, def]) => ({
    id: `nutrition_${key}`, source: "nutrition", mealKey: key,
...
function ResizeHandle({ color, onMouseDown, onTouchStart }) {
  return (
    <div data-resize="true" onMouseDown={onMouseDown} onTouchStart={onTouchStart}
      style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 16, cursor: "s-resize",
...
function NutritionBlock({ event, mealData, nutritionCompletion, onDragStart, onResizeStart, onClick, isDragging, isResizing }) {
  ...
  return (
    <div onMouseDown={pd} onTouchStart={pd}
      ...
      <ResizeHandle color={bothDone ? T.green : T.blue}
  ...
  return (
    <div onMouseDown={pd} onTouchStart={pd}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8, justifyContent: "space-between" }}>
        ...
        {tall && <span style={{ fontSize: 10, color: T.textFaint, flexShrink: 0, fontVariantNumeric: "tabular-nums", marginTop: 1 }}>{formatTime(event.startMinutes)}</span>}
      </div>
      {tall && (
    ...
    <div onClick={(e) => { e.stopPropagation(); onClick(); }}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 8, justifyContent: "space-between" }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: allDone ? T.greenText : T.redText, background: allDone ? "rgba(35,134,54,0.2)" : "rgba(218,54,51,0.15)", padding: "2px 8px", flexShrink: 0, fontVariantNumeric: "tabular-nums" }}>
          {totalCount > 0 ? `${doneCount}/${totalCount}` : "Tap to log"}
        </span>
      </div>
      <p style={{ fontSize: 11, color: T.textFaint, margin: 0 }}>
        {formatTime(9 * 60)} — Tap to log exercises
      </p>
    </div>
  );
}
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[
              ...
              ...(hyd != null ? [{ field: "hydrationDone", label: `Water — ${hyd} oz` }] : [{ field: "hydrationDone", label: "Hydration" }]),
            ].map(({ field, label }) => {
              const done = comp[field];
              return (
                <button key={field} onClick={() => onToggle(mealKey, field)}
                  style={{ flex: 1, padding: "12px 10px", border: `0.5px solid ${done ? T.green : T.border}`, background: done ? "rgba(35,134,54,0.12)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <div style={{ background: T.bgElevated, width: "100%", maxWidth: 480, maxHeight: "88vh", borderTop: `0.5px solid ${T.borderMid}`, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {sorted.map((item, idx) => {
            ...
            return (
              <div key={item.id || idx} onClick={() => onOpenItem?.(item)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px" }}>
                    {[item.Sets && `${item.Sets} sets`, item.Reps && `${item.Reps} reps`, item.Weight && item.Weight, item.Rest && `${item.Rest} rest`].filter(Boolean).map((s, i) => (
...
function EventModal({ event, defaultStartMinutes, onSave, onDelete, onClose, onOpenClassSchedule }) {
  ...
  useEffect(() => {
    ...
    if (!isTouch && inputRef.current && type !== "class") {
  ...
  const save = () => {
    if (type === "class") { onClose(); onOpenClassSchedule({ startMinutes: defaultStartMinutes }); return; }
        ...
        <div style={{ padding: "16px 18px 0", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {Object.entries(BLOCK_TYPES).map(([key, cfg]) => (
                <button key={key} onClick={() => setType(key)}
                  style={{ padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: `0.5px solid ${type === key ? cfg.border : T.border}`, background: type === key ? cfg.bg : "transparent", color: type === key ? cfg.title : T.textMuted, transition: "all 0.1s" }}>
                  {cfg.label}
                </button>
              ))}
            </div>
          ...
          {type === "class" && (
            <div style={{ background: T.bgBlock, border: `0.5px solid ${BLOCK_TYPES.class.border}`, padding: "16px" }}>
              ...
              <button onClick={() => { onClose(); onOpenClassSchedule({ startMinutes: defaultStartMinutes }); }}
                style={{ width: "100%", padding: "13px", border: "none", background: BLOCK_TYPES.class.border, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            ...
            <>
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any details..." rows={2}
                  style={{ ...sharedInput, resize: "none", boxSizing: "border-box", lineHeight: 1.6 }} />
              </div>
...
export default function DayPlannerSheet({
  ...
  onClose,
  classSchedules,
  onUpsertClass,   // (data, existingId | null) → void
  onRemoveClass,   // (id) → void
  authReady,
  user,
  isAthlete,
  athleteToken,
  ...
  useEffect(() => {
    if (isOpen && gridRef.current && todayFlag) {
      ...
      setTimeout(() => {
        gridRef.current?.scrollTo({ top: Math.max(0, minutesToY(now) - window.innerHeight / 3), behavior: "smooth" });
      }, 350);
    }
  }, [isOpen, todayFlag]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/athlete/nutrition/completion/upsert?date=${encodeURIComponent(dateStr)}`, { method: "GET", credentials: "include" })
      ...
      .then(data => {
        ...
        if (nutKey) lsSet(nutKey, JSON.stringify(n));
      })
      .catch(() => {})
  ...
  useEffect(() => {
    if (!authReady || !user || !isAthlete || !nutKey || nutHydRef.current) return;
    lsSet(nutKey, JSON.stringify(nutritionCompletion));
  ...
  // Seed nutrition defaults if no nutrition blocks exist yet
  useEffect(() => {
  ...
  const handleClassSave = useCallback((data) => {
    ...
    onUpsertClass(data, existingId);
    setClassModal(null);
    // Jump to next occurrence if current day doesn't match
    ...
    if (Array.isArray(data.days) && !data.days.includes(currentDow)) {
      ...
      while (offset <= 7) {
        ...
        if (data.days.includes(candidate.getDay())) { setCurrentDate(candidate); break; }
        offset++;
      }
    }
  ...
  const handleClassDelete = useCallback(() => {
    ...
    onRemoveClass(classModal.schedule.id);
    setClassModal(null);
  }, [classModal, onRemoveClass]);
  ...
  const nutritionTotal   = 8;
  const totalDone        = workoutDone + nutritionDone;
  const totalItems       = workoutTotal + nutritionTotal;
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ background: T.bg, borderBottom: `0.5px solid ${T.border}`, flexShrink: 0, paddingTop: "env(safe-area-inset-top, 0)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px 0" }}>
            <button onClick={onClose}
              style={{ background: "none", border: `0.5px solid ${T.border}`, color: T.textMuted, width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <X size={13} />
            </button>
            ...
            <div style={{ flex: 1, minWidth: 0, textAlign: "center" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 18, fontWeight: 700, color: T.textPrimary, letterSpacing: "-0.02em", whiteSpace: "nowrap" }}>
                  {todayFlag ? "Today" : currentDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
                </span>
                {todayFlag && (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px 10px" }}>
            ...
            {workoutTotal > 0 && <div style={{ width: "0.5px", height: 14, background: T.border }} />}
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <div style={{ width: 6, height: 6, background: T.green, flexShrink: 0 }} />
              <span style={{ fontSize: 13, fontWeight: 700, color: nutritionDone >= nutritionTotal ? T.greenText : T.textSecond, fontVariantNumeric: "tabular-nums" }}>
        ...
        <div
          ...
          style={{ flex: 1, overflowY: "auto", overflowX: "hidden", position: "relative", WebkitOverflowScrolling: "touch",
            // lock grid scroll when a sub-modal is open
            overflow: anySubModal ? "hidden" : undefined,
          }}
          ...
          >
            ...
            <div style={{ flex: 1, position: "relative", marginRight: 6 }}>
              ...
              {nutritionEvents.map(ev => (
                <NutritionBlock key={ev.id} event={ev}
              ...
              {/* Class blocks */}
              {classEvents.map(ev => {
                const sched = classSchedules.find(c => c.id === ev.scheduleId);
                return (
                  <ClassBlock key={ev.id} event={ev} schedule={sched}
                    onClick={() => sched && setClassModal({ schedule: sched })}
                  />
                );
              })}

              {/* One-off athlete blocks */}
        ...
        <button
          ...
          style={{
            ...
            cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 15,
          }}
      ...
      {/* ── Sub-modals (rendered outside sheet to guarantee z-index) ── */}
      {isOpen && modal && (
        <EventModal event={modal.event} defaultStartMinutes={modal.defaultStartMinutes}
          onSave={handleModalSave}
          onDelete={modal.mode === "edit" ? handleModalDelete : undefined}
          onClose={() => setModal(null)}
          onOpenClassSchedule={(opts) => { setModal(null); setClassModal({ schedule: null, defaultStartMinutes: opts.startMinutes }); }}
        />
      )}
      ...
      {isOpen && macroModal && (
        <MacroModal mealKey={macroModal.mealKey} mealData={mealBlocks?.[macroModal.mealKey]}
          event={macroModal.event} nutritionCompletion={nutritionCompletion}
          onToggle={handleNutritionToggle} onClose={() => setMacroModal(null)}
        />
      )}
      {isOpen && workoutModal && (
        <WorkoutDetailModal dailyWorkout={dailyWorkout} items={workoutItems}
          optimisticStatusById={optimisticStatusById}
      ...
      {isOpen && modalOpen && (
        <CompleteItemModal open={modalOpen} item={activeItem} selectedFile={selectedFile}
          coachNote={coachNote}
