...
function SmBtn({ children, onClick, variant = "secondary", fullWidth = false }) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (variant === "primary") {
          e.currentTarget.style.backgroundColor = DS.brandLight;
        } else {
...
export default function DaySheet({
  ...
  const handleEditFull = (editData) => {
    setDetailOpen(false);
    onClose?.(); // optional: closes the DaySheet before opening edit modal
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = DS.pageBg;
                  }}
                  onMouseLeave={(e) => {
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-5 space-y-2">
              {loading ? (
                <div className="py-8 text-center">
                  <p className="text-xs font-bold" style={{ color: DS.dimText }}>
                    Loading workouts…
                  </p>
                </div>
              ) : dayList.length === 0 ? (
