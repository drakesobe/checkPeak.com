...
export default function StackCard({
  ...
  maxCompare   = 3,
  maxSuppPills = 5,
  compact      = false, // ← true on mobile: shorter image, simplified body
  ...
  if (compact) {
    return (
      <motion.div
        ...
        style={{
          ...
          boxShadow:    isSelected
            ? "0 0 0 2px rgba(91,158,201,0.18), 0 4px 16px rgba(0,0,0,0.4)"
      ...
      >
        {/* Image — 3:2 ratio on compact to save vertical space */}
        <div className="relative w-full overflow-hidden" style={{ aspectRatio: "3/2" }}>
          {stack?.imageUrl ? (
            <>
              ...
              <div
                ...
                style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0) 45%, rgba(13,17,23,0.85) 100%)" }}
              />
            </>
          ) : (
          ...
          {/* Overlay row: compare button (left) + save (right) */}
          <div className="absolute top-2 left-2 right-2 flex items-center justify-between">
            {/* Compare — labeled pill */}
            <motion.button
              type="button"
              onClick={toggleCompare}
              className="flex items-center gap-1 rounded-lg px-2 py-1 transition-all"
              style={{
                background:     isSelected ? "#5B9EC9" : "rgba(13,17,23,0.75)",
                border:         isSelected ? "1px solid rgba(91,158,201,0.6)" : "1px solid rgba(255,255,255,0.18)",
                backdropFilter: "blur(6px)",
                boxShadow:      isSelected ? "0 0 10px rgba(91,158,201,0.35)" : "none",
              }}
              whileTap={{ scale: 0.92 }}
            ...
            >
              {isSelected ? (
                <svg viewBox="0 0 24 24" className="w-2.5 h-2.5 text-white shrink-0" fill="none" stroke="currentColor" strokeWidth={3} aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              ) : (
                <svg viewBox="0 0 24 24" className="w-2.5 h-2.5 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} style={{ color: "rgba(255,255,255,0.7)" }} aria-hidden="true">
              ...
              <span
                ...
                style={{ fontFamily: "'Barlow Condensed', sans-serif", color: isSelected ? "#fff" : "rgba(255,255,255,0.8)" }}
              >
          ...
          {stack?.category && (
            <div
              className="absolute bottom-2 left-2 rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest"
        ...
        {/* Card body — compact: name + value badge + single CTA */}
        <div className="flex flex-col px-2.5 pt-2 pb-2.5 gap-1.5">
          {/* Name */}
          <h3
            className="font-bold text-white leading-snug line-clamp-2"
            style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "clamp(0.78rem, 2.5vw, 0.9rem)", letterSpacing: "0.015em" }}
          >
            {stack?.name || "Untitled Stack"}
          </h3>

          {/* Price per serving — key comparison signal */}
          {priceLabel && (
            <p className="text-[11px] font-semibold tabular-nums" style={{ color: "rgba(255,255,255,0.55)", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.02em" }}>
              {priceLabel}
            </p>
          )}

          {/* Value badge */}
          <ValueBadge valueScore={stack?.valueScore} valueLabel={stack?.valueLabel} />
            ...
            {authToastOpen && (
              ...
              >
                <div className="px-2.5 py-2 flex items-center gap-2">
                  <FaLock size={9} style={{ color: "rgba(255,255,255,0.5)", shrink: 0 }} />
                  <p className="text-[11px] text-white font-semibold flex-1">Sign in to save</p>
                  <button type="button" onClick={() => setAuthToastOpen(false)} style={{ color: "rgba(255,255,255,0.3)" }} aria-label="Dismiss">
                    <FaTimes size={9} />
                  </button>
                </div>
                <div className="px-2.5 pb-2 flex gap-1.5">
                  <button type="button" className="flex-1 rounded-lg py-1.5 text-[11px] font-semibold text-white text-center" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }} onClick={() => openLogin("save_stack")}>
                    Sign in
                  </button>
                  <button type="button" className="flex-1 rounded-lg py-1.5 text-[11px] font-semibold text-white text-center" style={{ background: "#5B9EC9" }} onClick={() => openLogin("save_stack")}>
                    Register
                  </button>
                </div>
              </motion.div>
          ...
          <button
            ...
            style={{
              background:    canOpenModal ? "rgba(91,158,201,0.14)" : "rgba(255,255,255,0.03)",
              border:        canOpenModal ? "1px solid rgba(91,158,201,0.3)" : "1px solid rgba(255,255,255,0.05)",
              color:         canOpenModal ? "#5B9EC9" : "rgba(255,255,255,0.2)",
          ...
          {/* Amazon link — text-only on compact to save height */}
          {stack?.affiliateLink && (
            <a
    ...
    >
      ...
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: "4/3" }}>
        {stack?.imageUrl ? (
          <>
            ...
            <div
              aria-hidden="true"
              className="absolute inset-0"
              style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0) 40%, rgba(13,17,23,0.8) 100%)" }}
            />
          </>
        ) : (
        ...
        <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between">
          ...
          >
            {isSelected ? (
              <svg viewBox="0 0 24 24" className="w-3 h-3 text-white shrink-0" fill="none" stroke="currentColor" strokeWidth={3} aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
            ) : (
              <svg viewBox="0 0 24 24" className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} style={{ color: "rgba(255,255,255,0.7)" }} aria-hidden="true">
                <path strokeLinecap="round" d="M9 3H5a2 2 0 00-2 2v14a2 2 0 002 2h4M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4M9 3v18M15 3v18" />
              </svg>
            )}
      ...
      <div className="flex flex-col flex-1 px-3.5 pt-3 pb-3.5 gap-2.5">
        <div>
          <h3
            ...
            style={{ fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.02em", fontSize: "clamp(0.9rem, 1.5vw, 1.05rem)" }}
          >
            {stack?.name || "Untitled Stack"}
          </h3>
          {priceLabel && (
            <p className="mt-0.5 text-[12px] font-semibold tabular-nums" style={{ color: "rgba(255,255,255,0.45)", fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: "0.02em" }}>
              {priceLabel}
            </p>
          )}
        </div>
              ...
              <div className="px-3 py-2.5 flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button type="button" onClick={() => setAuthToastOpen(false)} className="shrink-0 -mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }} aria-label="Dismiss">
                      <FaTimes size={10} />
                    </button>
                  </div>
                  <div className="mt-2 flex gap-1.5">
                    <button type="button" className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-white transition-all" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }} onClick={() => openLogin("save_stack")}>
                      <FaSignInAlt size={9} aria-hidden="true" />
                      Sign in
                    </button>
                    <button type="button" className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-white transition-all" style={{ background: "#5B9EC9", border: "1px solid rgba(91,158,201,0.5)" }} onClick={() => openLogin("save_stack")}>
                      Create account
                    </button>
                  </div>
                </div>
        ...
        <div className="mt-auto pt-1 flex flex-col gap-1.5">
          ...
          {stack?.affiliateLink && (
            <a
              ...
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.55)", fontFamily: "'Barlow Condensed', sans-serif" }}
              onClick={(e) => {
                e.stopPropagation();
                if (typeof window.gtag === "function") {
                  window.gtag("event", "conversion", { send_to: "AW-17990566633/eJHlCOT724YcEOmFyYJD" });
