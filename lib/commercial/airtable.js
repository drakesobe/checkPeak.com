...
const TABLES = {
  ...
  purchases:     process.env.CLIENT_PURCHASES_TABLE_ID,
};
...
export async function getWorkoutById(recordId) {
  if (!TABLES.workouts) return null;
  const res = await fetch(atUrl(TABLES.workouts, `/${recordId}`), { headers: AT_HEADERS });
  if (!res.ok) return null;
  return res.json();
}

export async function getPurchasesByEmailAndTrainer(clientEmail, trainerId) {
  if (!TABLES.purchases) return [];
  const formula = `AND({clientEmail}="${clientEmail}", {trainerId}="${trainerId}", {status}="active")`;
  const url  = atUrl(TABLES.purchases, `?filterByFormula=${encodeURIComponent(formula)}`);
  const res  = await fetch(url, { headers: AT_HEADERS });
  const data = await res.json();
  return data.records ?? [];
}
...
export async function getPurchaseByEmailAndItem(clientEmail, itemId) {
  if (!TABLES.purchases) return null;
  const formula = `AND({clientEmail}="${clientEmail}", {itemId}="${itemId}", {status}="active")`;
  const url  = atUrl(TABLES.purchases, `?filterByFormula=${encodeURIComponent(formula)}`);
  const res  = await fetch(url, { headers: AT_HEADERS });
  const data = await res.json();
  return data.records?.[0] ?? null;
...
export async function createPurchase(fields) {
  if (!TABLES.purchases) return null;
  const res = await fetch(atUrl(TABLES.purchases), {
    method: "POST", headers: AT_HEADERS, body: JSON.stringify({ fields }),
  });
  return res.json();
}