...
const NUTRITION_JSONB = new Set(["planJson"]);

function toNutritionRow(camelObj) {
  const row = {};
  for (const [k, v] of Object.entries(camelObj)) {
    const snakeKey = camelToSnake(k);
    if (NUTRITION_JSONB.has(k) && typeof v === "string") {
      try { row[snakeKey] = JSON.parse(v); } catch { row[snakeKey] = v; }
    } else {
      row[snakeKey] = v;
    }
...
export async function getNutritionPlansByTrainer(trainerId, { publishedOnly = false } = {}) {
  let q = db.from("commercial_nutrition_plans").select("*").eq("trainer_id", trainerId).order("title");
  if (publishedOnly) q = q.eq("published", true);
  const { data } = await q;
  return (data ?? []).map(toRecord);
}

export async function getNutritionPlanById(recordId) {
  const { data } = await db.from("commercial_nutrition_plans").select("*").eq("id", recordId).maybeSingle();
  return toRecord(data);
}

export async function createNutritionPlan(fields) {
  const { data, error } = await db.from("commercial_nutrition_plans").insert(toNutritionRow(fields)).select().single();
  if (error) { console.error("[db] createNutritionPlan:", error); return { error }; }
  return toRecord(data);
}

export async function updateNutritionPlan(recordId, fields) {
  const { data, error } = await db.from("commercial_nutrition_plans").update(toNutritionRow(fields)).eq("id", recordId).select().single();
  if (error) { console.error("[db] updateNutritionPlan:", error); return { error }; }
  return toRecord(data);
}

export async function deleteNutritionPlan(recordId) {
  const { error } = await db.from("commercial_nutrition_plans").delete().eq("id", recordId);
  if (error) { console.error("[db] deleteNutritionPlan:", error); return { error }; }
  return { deleted: true };
}

// ─── Creator Agreements ───────────────────────────────────────────────────────

export async function getCreatorAgreementByTrainer(trainerId) {
  const { data } = await db
...
export async function getCreatorAgreementByCode(code) {
  const { data } = await db
    .from("creator_agreements")
    .select("*")
...
export async function getCreatorReferralCount(creatorCode) {
  const { count } = await db
    .from("trainers")
    .select("id", { count: "exact", head: true })
    .eq("referred_by_code", creatorCode);
  return count ?? 0;
}

export async function updateCreatorAgreementCode(trainerId, creatorCode) {
  const { data, error } = await db
    .from("creator_agreements")
    .update({ creator_code: creatorCode, code_changed_at: new Date().toISOString() })
    .eq("trainer_id", trainerId)
    .select()
    .single();
  if (error) { console.error("[db] updateCreatorAgreementCode:", error); return { error }; }
  return toRecord(data);
}

export async function createCreatorAgreement({ trainerId, legalName, email, creatorCode, signedAt }) {
  const { data, error } = await db
    .from("creator_agreements")
    .insert({ trainer_id: trainerId, legal_name: legalName, email, creator_code: creatorCode, signed_at: signedAt })
    .select()
    .single();
  if (error) { console.error("[db] createCreatorAgreement:", error); return { error }; }
  return toRecord(data);
}
