...
function asAthleteToken(v) {
  ...
  // ✅ enforce ATH- prefix (adjust if your prefix differs)
  if (!s) return "";
  if (!/^ATH-/i.test(s)) return ""; // reject ORG- and everything else
  return s;
}
...
export function requireAthlete(req) {
  ...
  // ✅ enforce proper AthleteToken (ATH-XXXX)
  const athleteToken = asAthleteToken(userRaw.AthleteToken || userRaw.athleteToken);
