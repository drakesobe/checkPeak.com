...
/**
 * Send to a single raw Expo push token (for org/coach users not in athletes table).
 */
export async function sendNotificationToToken(token, { title, body, data = {}, type = "" }) {
  if (!token || !token.startsWith("ExponentPushToken") || !title || !body) return;
  try {
    const channel = type === "new_message" ? "messages" : "default";
    fetch(EXPO_PUSH_URL, {
      method:  "POST",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify([{ to: token, title, body, data: { ...data, type }, sound: "default", channel }]),
    }).catch(() => {});
  } catch {}
}
