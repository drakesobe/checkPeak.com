...
export async function createOrg({ name, email, password, contactName, phone, website, address, notes, type, referredByCode }) {
  const password_hash = await hashPassword(password);
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      referred_by_code: referredByCode ? String(referredByCode).toUpperCase().trim() : null,
