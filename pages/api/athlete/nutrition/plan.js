...
const ATHLETE_TABLE_NAME = process.env.ATHLETE_TABLE_NAME; // AthleteScans table id/name
...
const PLAN_DPRO = "DailyProtein"; // ✅ note exact casing
const PLAN_DCARB = "DailyCarbs";  // ✅ note exact casing
...
export default async function handler(req, res) {
  ...
  // Athlete identity from session cookie payload
  const sessionTok = asString(auth?.athlete?.athleteToken || auth?.athlete?.token || auth?.athlete?.AthleteToken);
  ...
  // Optional query param (ignored if it doesn't match session)
  ...
  try {
    /* ---------------- 1) Find Athlete record by session token ---------------- */

    const athleteFilter = tokenMatchFormula(ATH_TOKEN, sessionTok);
    ...
    /* ---------------- 2) Latest ACTIVE plan for this athlete ---------------- */
    ...
    if (latestRec) {
      ...
      latestPlan = {
        ...
        // Structured fields (Airtable single line text is fine; we just pass through)
